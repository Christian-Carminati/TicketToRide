...
def test_recurrent_actor_critic_shapes_single_step():
    model = RecurrentMaskedActorCritic(
        input_dim=50, action_dim=20, hidden_dim=64, lstm_hidden_dim=64
    )
...
def test_recurrent_actor_critic_action_masking_enforcement():
    model = RecurrentMaskedActorCritic(
        input_dim=50, action_dim=5, hidden_dim=64, lstm_hidden_dim=64
    )
    obs = torch.randn(1, 50)
    hidden = model.get_initial_hidden(batch_size=1)
    ...
    action, _log_prob, _entropy, _value, _ = model.get_action_and_value(
        obs, hidden, action_mask=mask
    )
...
def test_recurrent_actor_critic_deterministic_action():
    model = RecurrentMaskedActorCritic(
        input_dim=10, action_dim=4, hidden_dim=32, lstm_hidden_dim=32
    )
...
def test_recurrent_actor_critic_provided_action_evaluation():
    ...
    action, log_prob, entropy, _value, _ = model.get_action_and_value(
...
def test_recurrent_actor_critic_non_orthogonal_init():
    ...
    hidden = model.get_initial_hidden(batch_size=1)
    action, _log_prob, _entropy, _value, _new_hidden = model.get_action_and_value(obs, hidden)
    assert action.shape == (1,)
