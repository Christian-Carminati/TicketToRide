...
class AlphaZeroTrainer:
    ...
    def __init__(
        ...
        self.search_engine = NeuralMCTSEngine(
            net=self.net, num_simulations=num_simulations, c_puct=c_puct, board=board, tickets=tickets
        )
        self.board = board
        self.encoder = (
    ...
    def _get_obs_and_mask(self, game: Game, player_id: str) -> tuple[np.ndarray, np.ndarray]:
        ...
        if self.board is None or (
            ...
            self.encoder = ObservationV1(board=self.board, initial_tickets=tickets)
    ...
    def train_step(self, batch_size: int | None = None) -> dict[str, float]:
        ...
        self.optimizer.step()
        self.net.sync_fast_evaluator()
        if hasattr(self.search_engine, "tt_cache"):
            self.search_engine.tt_cache.clear()
