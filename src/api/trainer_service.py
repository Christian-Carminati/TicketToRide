...
from src.rl.self_play import PolicyPool, SelfPlayPPOTrainer
...
class TrainerService:
    ...
    def _run_training(
        ...
        if algo in ("recurrent_ppo", "lstm_ppo"):
            ...
            # Lesson 9: Self-Play Policy Pool PPO Trainer
            policy_pool = PolicyPool(capacity=10)
            sp_config = {
                "lr": 3e-4,
                "gamma": 0.99,
                "rollout_steps": 64,
            ...
            while step < total_steps and not self._stop_requested:
                ...
                self._status.current_step = min(step, total_steps)
                self._status.episodes = episode_count
                self._status.mean_reward = float(metrics.get("mean_reward", 0.0))
