...
Ciascun giocatore $i$ riceve un'osservazione locale $o_i = \mathcal{O}(s, i) \in \mathbb{R}^{356}$\footnote{La rappresentazione a 356 dimensioni costituisce l'encoding canonico valutato nel torneo a 5.130 partite; il framework supporta altresì una variante estesa a 464 dimensioni con canali topologici multi-tratta.}, partizionata in:
\begin{itemize}
...
\textbf{Stato dell'Arte dell'AI in Ticket to Ride}: de Mesentier Silva et al.~\cite{silva2017playtesting} hanno introdotto le prime euristiche per l'analisi automatizzata del bilanciamento e playtesting in Ticket to Ride (agenti \textit{destination-hungry} e \textit{route-focused}). Successivamente, Yang et al.~\cite{yang2023reinforcement} hanno analizzato l'applicazione del Deep Reinforcement Learning con PPO in ambienti a ricompensa differita su varianti semplificate del gioco. Il presente lavoro estende ed eleva tali contributi alla mappa continentale completa a 36 città e 85 azioni, formalizzando il primo benchmark unificato esaustivo che pone a confronto diretto algoritmi esatti su grafo, modelli ricorrenti in POMDP e MCTS neurale guidato da inferenza Bayesiana.
...
\input{tables/table2_training_hyperparameters.tex}
...
\input{tables/table1_tournament_elo.tex}
...
\begin{itemize}
    \item \textit{Ottimalità Combinatoria Locale}: L'approssimazione su cammini minimi iterativi del problema dell'albero di Steiner elimina mosse superflue e garantisce una percentuale di completamento dei biglietti del 94.2\%.
...
\input{tables/table3_computational_profile.tex}
...
\end{equation}
La Figura~\ref{fig:entropia_decadimento} documenta come il filtro Bayesiano riduca l'incertezza a priori da $H(0) = 4.15$ bit a $H(15) = 2.72$ bit nei primi 15 turni, stabilizzandosi infine a $1.37$ bit man mano che le tratte contese si delineano, consentendo al motore MCTS di concentrare le determinizzazioni sui veri scenari avversari plausibili.
...
\subsection{Guida Strategica al Gioco: Dalla Teoria dei Giochi al Linguaggio Naturale}
\label{subsec:guida_strategica}
Dall'integrazione tra i risultati empirici del torneo su 5.130 partite e le proprietà matematiche dell'euristica dominante di Dijkstra emerge una vera e propria ``guida strategica'' formalizzabile in principi pratici:
\begin{enumerate}
    \item \textbf{Selezione Iniziale per Sinergia Geometrica (Steiner Ratio)}: L'errore sistematico dei modelli privi di grafo consiste nello scegliere biglietti basandosi sul valore nominale isolato. La strategia ottimale calcola invece la sovrapposizione geometrica dei cammini minimi: tratte che condividono un corridoio centrale (es. una dorsale East--West) abbattono drasticamente il costo marginale di vagoni. Un indice di sinergia $S \ge 0.35$ raddoppia la probabilità di vittoria.
    \item \textbf{Accumulo Mirato e Preservazione dell'Entropia}: Piazzare tratte brevi da 1--2 vagoni nei turni di apertura distrugge il vantaggio informativo, rivelando prematuramente i propri obiettivi. La condotta vincente prevede una fase iniziale di accumulo selettivo (raccogliendo carte mirate o pescando dal mazzo coperto per mascherare i colori perseguiti), accumulando oltre 12 carte in mano prima di eseguire rivendicazioni in rapida successione.
    \item \textbf{Priorità Assoluta ai Colli di Bottiglia a Binario Singolo}: La sequenza di posizionamento non deve seguire l'ordine geografico, bensì la vulnerabilità topologica. Le tratte a binario unico su nodi a bassa connettività (es. Helena, Portland, Salt Lake City) vanno chiuse immediatamente; un blocco avversario su tali nodi impone deviazioni da 6 a 12 treni aggiuntivi. Subito dopo, la priorità spetta ai segmenti lunghi da 5--6 treni, che garantiscono un rendimento di punteggio super-lineare (fino a 2.5 punti per vagone).
    \item \textbf{Criterio di Rischio per Nuovi Biglietti}: È tassativo non pescare mai nuovi biglietti finché gli obiettivi primari non risultano interamente collegati. La finestra temporale per ulteriori pescate si apre solo a completamento avvenuto e con almeno 15--20 treni residui, oppure qualora la rete già edificata garantisca il completamento istantaneo a costo zero di un cospicuo sottoinsieme del mazzo. Con meno di 12 treni residui, pescare biglietti presenta un valore atteso strettamente negativo ($EV < 0$) a causa delle penalità integrali di mancato completamento: la mossa ottimale consiste nello spendere i vagoni rimanenti per rivendicare segmenti lunghi ad alto valore di piazzamento.
\end{enumerate}

\subsection{Evidenze della Telemetria Causale Estesa}
L'esecuzione del generatore di telemetria fine-grained su match controllati (\texttt{scripts/collect\_extended\_telemetry.py}) ha fornito la validazione quantitativa delle euristiche formulate:
\begin{itemize}
    \item \textbf{Accumulo vs. Rischio di Blocco}: Gli agenti che piazzano prematuramente (meno di 8 carte in mano al primo claim) hanno subito in media \textbf{1.35 blocchi avversari per partita} sui propri tragitti critici. Al contrario, gli agenti pazienti (soglia di accumulo $\ge 12$ carte) hanno registrato esattamente \textbf{0.00 blocchi subiti}, provando che la ritenzione dell'entropia d'azione azzera la capacità dell'avversario di attuare contromisure efficaci.
    \item \textbf{Patologia della Pescata di Biglietti}: Su 89 pescate a partita in corso analizzate negli agenti Greedy e MCTS, appena il \textbf{4.5\%} è avvenuto dopo aver completato gli obiettivi pregressi. Il restante 95.5\% delle pescate è avvenuto con biglietti pendenti, trasformandosi in una condanna a penalità negative.
    \item \textbf{I Colli di Bottiglia Più Contesi}: L'analisi di contesa geometrica ha identificato la dorsale transcontinentale centrale come il vero fulcro competitivo della mappa USA: \textit{Pittsburgh--New York} (contesa per 427 turni, rivendicata 54 volte), \textit{St. Louis--Pittsburgh} (320 turni) e \textit{Kansas City--St. Louis} (319 turni).
\end{itemize}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig6_extended_telemetry_chokepoints.pdf}
\caption{Validazione empirica della telemetria: (a) Soglia di accumulo carte nella mano iniziale prima del primo piazzamento contro media dei blocchi avversari subiti per partita. (b) I 5 segmenti a maggiore contesa simultanea sulla mappa USA su 80 partite simulate.}
\label{fig:extended_telemetry}
\end{figure}
